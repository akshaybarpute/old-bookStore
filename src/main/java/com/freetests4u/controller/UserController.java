...
public class UserController {
	...
	public ResponseEntity<GenericResponseObject<User>> addUser(@RequestBody CreateUserRequest userRequest){
		...
		return new ResponseEntity<>(new GenericResponseObject<>(userRequest.getUser(), "created", false), HttpStatus.OK);
		}
		catch(Exception e) {
			System.out.println("inside the add user catching");
			e.printStackTrace();
			return new ResponseEntity<>(new GenericResponseObject<>(null, e.getMessage(),true), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	public ResponseEntity<GenericResponseObject<UserLoginResponse>> getUser(@RequestBody LoginPasswordRequest request){
		...
		try {
			...
			return new ResponseEntity<>(new GenericResponseObject<>(user,"success",false),HttpStatus.OK);
		}
		catch(Exception e) {
			e.printStackTrace();
